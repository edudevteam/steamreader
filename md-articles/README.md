...
A course is a collection of articles grouped together as a structured learning path. Courses are displayed on the homepage and each course has its own dedicated page at `/course/<slug>`.

### Course Data File

Courses are defined in `public-site/src/data/courses.json`. This file is **manually maintained** (not generated by the build scripts).

### Course Fields

| Field | Type | Description |
|-------|------|-------------|
| `slug` | string | URL-safe identifier, used in the route `/course/<slug>` |
| `title` | string | Display name for the course |
| `description` | string | Short summary shown on the course card |
| `featureImage.src` | string | Path to the course card image (e.g., `/images/my-course.png`) |
| `featureImage.alt` | string | Alt text for the course image |
| `articles` | string[] | Ordered list of article slugs that make up the course |

### Adding a New Course

1. Add an entry to `public-site/src/data/courses.json`:
  ...
  "courses": [
    {
      "slug": "my-course",
      "title": "My Course",
      "description": "A short description of what this course covers.",
      "featureImage": {
        "src": "/images/my-course.png",
        "alt": "My Course"
      },
      "articles": [
        "first-article-slug",
...
2. Make sure each article slug in the `articles` array matches an existing published article.
3. The order of slugs in the `articles` array determines the lesson order on the course page.
4. Place the course feature image in `md-articles/content/images/` and run the build so it gets copied to `public-site/public/images/`.

### How Courses Are Displayed

- **Homepage**: A "Courses" section shows course cards in a grid (default limit of 3). Each card links to `/course/<slug>`.
- **Course Page** (`/course/<slug>`): Lists all articles in the course as numbered lesson cards, preserving the order from the `articles` array.

### Courses vs. Article Series

- **Article Series** (`prev`/`next` frontmatter fields): Creates sequential navigation links between individual articles. This is used within articles for "Previous / Next" buttons.
- **Courses** (`courses.json`): Groups articles into a named collection with its own page and homepage card. A course's articles should typically also use `prev`/`next` for in-article navigation.
